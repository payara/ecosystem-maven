...
### Latest version available: 1.0.8-SNAPSHOT
...
This goal bundles the attached project's artifact into uber jar with specified configurations. ```bundle``` is attached to the ```install``` phase by default. A sample usage would as follows:
    ...
    <plugin>
        ...
        <configuration>
            ...
            <payaraVersion>5.2020.3</payaraVersion>
            <systemProperties>
                <property>
...
- __payaraVersion__ (optional |  default: 5.2020.3): By default ```bundle``` mojo fetches payara-micro with version 5.2020.3.
...
- __payaraVersion__ (optional | default: "5.2020.3"): The payara-micro version that will be used with ```start``` mojo.
